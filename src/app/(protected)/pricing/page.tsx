...
/* ── Design tokens (Adaptive Light & Dark CC·AI Palette) ── */
const C = {
  bg: "var(--pricing-bg, transparent)",
  card: "var(--pricing-card-bg, #FFFFFF)",
  cardHi: "var(--pricing-card-popular-bg, #FFFFFF)",
  border: "var(--pricing-card-border, rgba(0,0,0,0.08))",
  violet: "var(--violet, #4D3FFF)",
  violetLight: "var(--logo-accent, #7B70FF)",
  jade: "#00C896",
  ink: "var(--ink, #1a1a2e)",
  mist: "var(--mist, #6B7280)",
  faint: "var(--textLight, #94A3B8)",
  shadow: "var(--pricing-card-shadow, 0 10px 30px rgba(0,0,0,0.04))",
  shadowHi: "var(--pricing-card-popular-shadow, 0 20px 45px rgba(77,63,255,0.14))",
  toggleBg: "var(--pricing-toggle-bg, rgba(0,0,0,0.05))",
  btnBg: "var(--pricing-btn-bg, rgba(77,63,255,0.08))",
  btnColor: "var(--pricing-btn-color, #4D3FFF)",
  quoteBg: "var(--pricing-quote-bg, rgba(0,200,150,0.06))",
...
const FALLBACK_PLANS: Plan[] = [
  { id: "survival-pack", name: "Survival Pack", monthlyPrice: 49, annualPrice: 399, premiumDays: 30 },
  { id: "cgpa-builder", name: "CGPA Builder", monthlyPrice: 99, annualPrice: 799, premiumDays: 30 },
  { id: "topper-list", name: "Topper List", monthlyPrice: 199, annualPrice: 1499, premiumDays: 30 },
];

const FALLBACK_TOPUPS: Topup[] = [
  { id: "quick-boost", name: "Quick Boost", price: 9, accessHours: 12 },
  { id: "assignment-rescue", name: "Assignment Rescue", price: 19, accessHours: 24 },
  { id: "exam-rescue", name: "Exam Rescue", price: 39, accessHours: 48 },
];

export default function PricingPage() {
  const router = useRouter();
  const [plans, setPlans] = useState<Plan[]>(FALLBACK_PLANS);
  const [topups, setTopups] = useState<Topup[]>(FALLBACK_TOPUPS);
  ...
  const fetchPackages = async () => {
    try {
      ...
      if (cat && typeof cat === "object" && (Array.isArray(cat.plans) || Array.isArray(cat.topups))) {
        ...
        if (t.length) setTopups(t);
      }
    } catch (err) {
      console.warn("Using fallback pricing catalogue:", err);
    }
  };
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", paddingBottom: 80 }}>
      ...
      {toast && (
        <div style={{ position: "fixed", bottom: 32, right: 32, zIndex: 10000, background: toast.type === "error" ? "#FF4D5A" : C.violet, color: "#fff", padding: "14px 18px", borderRadius: 14, fontFamily: FONT.body, fontWeight: 600, boxShadow: "0 16px 36px rgba(0,0,0,0.25)", border: `1px solid ${C.border}`, maxWidth: 360 }}>
          {toast.msg}
        </div>
      )}
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: isMobile ? "32px 20px 0" : "48px 40px 0" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 40 }}>
          <span style={{ fontFamily: FONT.mono, fontSize: 12, letterSpacing: "0.14em", color: C.violetLight, textTransform: "uppercase", fontWeight: 700 }}>
            Recharge · 2025–26
          </span>
        ...
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          ...
          {/* Billing toggle */}
          <div style={{ display: "inline-flex", marginTop: 28, background: C.toggleBg, borderRadius: 999, padding: 5, border: `1px solid ${C.border}` }}>
          ...
          {plans.map((plan, idx) => {
            ...
            return (
              <motion.div key={plan.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.08 }}
                style={{
                  position: "relative",
                  background: highlight ? C.cardHi : C.card,
                  borderRadius: 22,
                  padding: isMobile ? 26 : 30,
                  border: highlight ? `2px solid ${C.violet}` : `1px solid ${C.border}`,
                  display: "flex",
                  flexDirection: "column",
                  boxShadow: highlight ? C.shadowHi : C.shadow,
                ...
                <button onClick={() => startPayment({ itemId: plan.id, type: "plan", amount: shownPrice, billingMode: yearly ? "yearly" : "monthly", name: plan.name, successLabel: `${plan.name} (${period})` })} disabled={buying !== null}
                  style={{
                    ...
                    background: highlight ? C.violet : C.btnBg,
                    color: highlight ? "#fff" : C.btnColor,
                ...
                {/* Student feeling */}
                <div style={{ marginTop: 22, padding: "14px 16px", background: C.quoteBg, borderRadius: 12, border: `1px solid ${C.quoteBorder}` }}>
                  <span style={{ fontFamily: FONT.display, fontStyle: "italic", fontSize: 14, color: "#00B386" }}>"{copy.feeling}"</span>
                </div>
              </motion.div>
            );
          })}
          ...
          {topups.map((t, idx) => {
            ...
            return (
              <motion.div key={t.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.08 }}
                style={{ background: C.card, borderRadius: 20, padding: 26, border: `1px solid ${C.border}`, display: "flex", flexDirection: "column", boxShadow: C.shadow }}>
                ...
                <button onClick={() => startPayment({ itemId: t.id, type: "topup", amount: t.price, billingMode: "monthly", name: t.name, successLabel: `${t.name} (${t.accessHours}h)` })} disabled={buying !== null}
                  style={{ width: "100%", padding: 13, borderRadius: 12, border: `1.5px solid ${C.violet}`, background: "transparent", color: C.violet, fontFamily: FONT.body, fontSize: 14, fontWeight: 700, cursor: buying ? "not-allowed" : "pointer", transition: "all 0.2s" }}>
