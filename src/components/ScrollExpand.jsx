...
const ScrollExpand = ({
  ...
  const applyProgress = useCallback(p => {
    ...
    if (borderRef.current) {
      borderRef.current.style.left = `${ix}%`;
      borderRef.current.style.top = `${iy}%`;
      borderRef.current.style.width = `${w}%`;
      borderRef.current.style.height = `${h}%`;
    ...
    if (scrimRef.current) {
      scrimRef.current.style.opacity = `${0.3 + (c.overlayScrim - 0.3) * e}`;
    }
